...
preditor <- c("RI", "Na", "Mg", "Al", "Si","K", "Ca", "Ba", "Fe", "Tipo")
medidas = matrix(1,nrow = 3 , ncol = 9)
colnames(medidas) = c("RI","Na","Mg","Al","Si","K","Ca","Ba","Fe")
rownames(medidas) = c("media","DP","Simetria")
for (i in 1:9){
	medidas[1,i] = round(mean(Glass[,i]),3)
	medidas[2,i] = round(sd(Glass[,i]),3)
	medidas[3,i] = round(skewness(Glass[,i]),3)
}

for (i in 1:9){
	ggplot(Glass,aes(Glass[,i])) + geom_histogram(color='black',fill='springgreen2')+theme_gray()+labs(title=paste("Histograma ", preditor[i]),x= paste("Porcentagem",preditor[i]),y="Frequência")
	ggsave(paste( "Histograma" , preditor[i] , ".png" , sep='' ))
}
dev.off()		
...
for (i in unique(Glass$Type)){ # os tipos
	for( j in 1:9){ # pegas os tipos
		condClassPred = Glass[Glass$Type==i,]
		ggplot( condClassPred , aes(condClassPred[,j])) + geom_histogram(color='black',fill='springgreen2') + theme_gray() + labs(title=paste("Histograma ", preditor[j],", Classe ",toString(i)), x=paste("Porcentagem",preditor[j]),y="Frequência")
		ggsave(paste("Histograma" , preditor[j] , "_" , "Classe", toString(i) , ".png" , sep='' ))
	}
}
dev.off()

medidasCC = array(rep(1,3*9*7), dim=c(3,9,7))
colnames(medidasCC) = c("RI","Na","Mg","Al","Si","K","Ca","Ba","Fe")
rownames(medidasCC) = c("media","DP","Simetria")
	
for (i in 1:7){
	for (j in 1:9){
		medidasCC[1,j,i] = round( mean(Glass[Glass$Type==i,j]) ,3)
		medidasCC[2,j,i] = round( sd(Glass[Glass$Type==i,j]) ,3)
		medidasCC[3,j,i] = round( skewness(Glass[Glass$Type==i,j]) ,3)
	}
}

print(Glass[(Glass$K == 0 & Glass$Ba == 0 & Glass$Fe == 0),]) # análise interessante 1, Classe 6?
print(Glass[(Glass$Mg == 0),]) # análise interessante 2, Presença de Magnésio
...
for(i in 1:9){ # classes 
	for(j in 1:9){ # classes
		if(i>j){
			ggplot( Glass ,aes(Glass[,j],Glass[,i],colour=as.factor(Tipo)))+ geom_point() +labs(x= preditor[j], y=preditor[i],colour="Tipo")
			ggsave(paste("ScatterPlot_",preditor[i]," vs ",preditor[j],".png", sep = ''))
		}
	}
}
dev.off()
...
matrix <- matrix(data=c(1,1.52101,13.64,4.49,1.10,71.78,0.06,8.75,0.00,0.00,1,
                              ...
                              214,1.51711,14.23,0.00,2.08,73.36,0.00,8.62,1.67,0.00,7),nrow=11,ncol=214)

rownames(matrix) <- c("ID" , "RI", "Na", "Mg", "Al", "Si","K", "Ca", "Ba", "Fe", "Tipo")

GlassDF <- as.data.frame(t(matrix[2:11,]))

CorrPred = cor(GlassDF[,1:9])

png("Matriz_Correlação_mixed.png")
corrplot.mixed(CorrPred)
dev.off()

png("Matriz_Correlação_circle.png")
corrplot(CorrPred , method = "circle")
dev.off()
...
pcaGlass = prcomp(GlassDF,scale=TRUE)

png("VariânciavsPC1&2.png")
fviz_eig(pcaGlass ,xlab="PC",ylab="Variância",ncp=2,addlabels = TRUE,main="Variância vs Componentes")
dev.off()
png("VariânciavsPCs.png")
fviz_eig(pcaGlass ,xlab="PC",ylab="Variância",addlabels = TRUE,main="Variância vs Componentes")
dev.off()


PC = data.frame(pcaGlass$x[,1],pcaGlass$x[,2])
PC["Tipo"] = Glass["Type"]
ggplot() + geom_point(data=PC, aes(x=PC[,1], y=PC[,2],color= as.factor(Tipo))) +labs(color='Tipo',x="PC1",y="PC2")
ggsave("PC1vsPC1.png")
dev.off()
